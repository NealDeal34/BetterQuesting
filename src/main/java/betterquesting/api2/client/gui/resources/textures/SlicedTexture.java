...
public class SlicedTexture implements IGuiTexture {
    ...
    public void drawTexture(int x, int y, int width, int height, float zLevel, float partialTick, IGuiColor color) {
        ...
        drawTexture(x, y, width, height, zLevel, partialTick,
            color.getRed(), color.getGreen(), color.getBlue(), color.getAlpha());
    }

    private void drawTexture(int x, int y, int width, int height, float zLevel, float partialTick,
        ...
        if (sliceMode == SliceMode.SLICED_TILE) {
            ...
            GuiUtils.drawTexturedModalRect(0, 0, texBounds.getX(), texBounds.getY(),
    ...
    private static void drawContinuousTexturedBox(ResourceLocation res, int x, int y, int u, int v, int width,
        ...
        for (int i = 0; i < xPasses + (remainderWidth > 0 ? 1 : 0); i++) {
            ...
            GuiUtils.drawTexturedModalRect(
                ...
                u + leftBorder, v, w, topBorder, zLevel);
            GuiUtils.drawTexturedModalRect(
                x + leftBorder + (i * fillerWidth), y + topBorder + canvasHeight,
                u + leftBorder, v + topBorder + fillerHeight, w, bottomBorder, zLevel);
