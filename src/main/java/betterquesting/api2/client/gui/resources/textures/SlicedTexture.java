...
public class SlicedTexture implements IGuiTexture {
        ...
        try {
            ...
            if (sliceMode == SliceMode.SLICED_TILE) {
                drawContinuousTexturedBox(
                    texture, dx, dy,
                    texBounds.getX(), texBounds.getY(),
                    w, h,
                    texBounds.getWidth(), texBounds.getHeight(),
                    texBorder.getTop(), texBorder.getBottom(),
                    texBorder.getLeft(), texBorder.getRight(),
                    zLevel);
                ...
                drawCorner(dx, dy, texBounds.getX(), texBounds.getY(), 
                    texBorder.getLeft(), texBorder.getTop(), zLevel);
                drawCorner(dx + texBorder.getLeft(), dy, 
                    texBounds.getX() + texBorder.getLeft(), texBounds.getY(),
                    iw, texBorder.getTop(), zLevel, sx, 1F);
                drawCorner(dx + w - texBorder.getRight(), dy,
                    texBounds.getX() + texBorder.getLeft() + iw, texBounds.getY(),
                    texBorder.getRight(), texBorder.getTop(), zLevel);
                drawCorner(dx, dy + texBorder.getTop(),
                    texBounds.getX(), texBounds.getY() + texBorder.getTop(),
                    texBorder.getLeft(), ih, zLevel, 1F, sy);
                drawCorner(dx + texBorder.getLeft(), dy + texBorder.getTop(),
                    iu, iv, iw, ih, zLevel, sx, sy);
                drawCorner(dx + w - texBorder.getRight(), dy + texBorder.getTop(),
                    texBounds.getX() + texBorder.getLeft() + iw, texBounds.getY() + texBorder.getTop(),
                    texBorder.getRight(), ih, zLevel, 1F, sy);
                drawCorner(dx, dy + h - texBorder.getBottom(),
                    texBounds.getX(), texBounds.getY() + texBorder.getTop() + ih,
                    texBorder.getLeft(), texBorder.getBottom(), zLevel);
                drawCorner(dx + texBorder.getLeft(), dy + h - texBorder.getBottom(),
                    texBounds.getX() + texBorder.getLeft(), texBounds.getY() + texBorder.getTop() + ih,
                    iw, texBorder.getBottom(), zLevel, sx, 1F);
                drawCorner(dx + w - texBorder.getRight(), dy + h - texBorder.getBottom(),
                    texBounds.getX() + texBorder.getLeft() + iw, texBounds.getY() + texBorder.getTop() + ih,
                    texBorder.getRight(), texBorder.getBottom(), zLevel);
                ...
                Minecraft.getMinecraft().getTextureManager().bindTexture(texture);
                GuiUtils.drawTexturedModalRect(0, 0,
                    texBounds.getX(), texBounds.getY(),
                    texBounds.getWidth(), texBounds.getHeight(), zLevel);
            }
    ...
    private static void drawContinuousTexturedBox(
        ResourceLocation res, int x, int y, int u, int v, int width, int height,
        ...
        for (int j = 0; j < yPasses + (remainderHeight > 0 ? 1 : 0); j++) {
            ...
            GuiUtils.drawTexturedModalRect(
                ...
                u, v + topBorder, leftBorder, drawH, zLevel);
            GuiUtils.drawTexturedModalRect(
                x + leftBorder + canvasWidth, y + topBorder + j * fillerHeight,
                u + leftBorder + fillerWidth, v + topBorder,
                rightBorder, drawH, zLevel);
        }
    }

    public enum SliceMode { STRETCH, SLICED_TILE, SLICED_STRETCH }
}
