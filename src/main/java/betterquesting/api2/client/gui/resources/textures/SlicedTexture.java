...
public class SlicedTexture implements IGuiTexture {
    ...
    public void drawTexture(int x, int y, int width, int height, float zLevel, float partialTick, IGuiColor color) {
        ...
        GL11.glPushAttrib(GL11.GL_COLOR_BUFFER_BIT | GL11.GL_ENABLE_BIT | GL11.GL_CURRENT_BIT | GL11.GL_TEXTURE_BIT);
        GL11.glPushMatrix();
        ...
        try {
            ...
            if (sliceMode == SliceMode.SLICED_TILE) {
                ...
                float sx = (iw > 0) ? (float) (w - texBorder.getLeft() - texBorder.getRight()) / (float) iw : 1F;
                float sy = (ih > 0) ? (float) (h - texBorder.getTop() - texBorder.getBottom()) / (float) ih : 1F;
