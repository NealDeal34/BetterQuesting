        ...
        try {
            ...
            if (sliceMode == SliceMode.SLICED_TILE) {
                ...
                GuiUtils.drawTexturedModalRect(0, 0, texBounds.getX() + texBorder.getLeft(), 
                    texBounds.getY(), iw, texBorder.getTop(), zLevel);
                GL11.glPopMatrix();
                ...
                GuiUtils.drawTexturedModalRect(0, 0, texBounds.getX() + texBorder.getLeft() + iw, 
                    texBounds.getY(), texBorder.getRight(), texBorder.getTop(), zLevel);
                GL11.glPopMatrix();
                ...
                GuiUtils.drawTexturedModalRect(0, 0, texBounds.getX(), 
                    texBounds.getY() + texBorder.getTop(), texBorder.getLeft(), ih, zLevel);
                GL11.glPopMatrix();
                ...
                GuiUtils.drawTexturedModalRect(0, 0, texBounds.getX(), 
                    texBounds.getY() + texBorder.getTop() + ih, texBorder.getLeft(), 
    ...
    private static void drawContinuousTexturedBox(ResourceLocation res, int x, int y, int u, int v, int width,
        ...
        for (int i = 0; i < xPasses + (remainderWidth > 0 ? 1 : 0); i++) {
            ...
            GuiUtils.drawTexturedModalRect(
                ...
                (i == xPasses ? remainderWidth : fillerWidth), bottomBorder, zLevel);
        ...
        for (int j = 0; j < yPasses + (remainderHeight > 0 ? 1 : 0); j++) {
            GuiUtils.drawTexturedModalRect(
                ...
                u, v + topBorder,
                leftBorder, (j == yPasses ? remainderHeight : fillerHeight), zLevel);
            GuiUtils.drawTexturedModalRect(
                x + leftBorder + canvasWidth, y + topBorder + (j * fillerHeight),
                u + leftBorder + fillerWidth, v + topBorder,
                rightBorder, (j == yPasses ? remainderHeight : fillerHeight), zLevel);
        }
    }
